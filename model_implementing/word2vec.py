...
def find_most_similar_text(model: gensim.models.keyedvectors.KeyedVectors, query: str, texts: list[str], top_k: int) -> list[str]:
    ...
    return texts_vectors.nlargest(n = top_k, columns='query_distance')['text'].to_list(), model.cosine_similarities(query_vector, list(texts_vectors['text_vector']))
